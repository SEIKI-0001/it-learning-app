...
- `fequest:pastExam:*` is the complete local resume frame. Authenticated exam
  ...
  `/api/question-attempts/save`, `/api/progress/save`, and, on applicable
  surfaces, `/api/answers/save`. These remote rows do not reconstruct the local
  cursor, timer, answer snapshot, or pending mutation.
