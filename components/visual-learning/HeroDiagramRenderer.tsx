  ...
  return (
    <figure className="overflow-hidden rounded-lg border border-gray-200 bg-slate-50 shadow-sm">
      <div className="border-b border-white/70 bg-white px-4 py-3">
        ...
        <figcaption className="text-base font-bold leading-snug text-gray-950">
          {spec.title}
        </figcaption>
        {spec.subtitle && (
...
function NodePill({ node }: { node: HeroDiagramNode }) {
  ...
  return (
    <div className={`rounded-lg border ${style.border} ${style.surface} p-3`}>
      {node.badge && (
        <p className={`mb-2 inline-flex rounded-full px-2 py-0.5 text-[11px] font-bold ${style.badge}`}>
          {node.badge}
        </p>
      )}
      <p className={`text-sm font-bold ${style.text}`}>{node.label}</p>
      {node.caption && (
...
function LanePanel({ lane }: { lane: HeroDiagramLane }) {
  ...
  return (
    <div className={`rounded-lg border ${style.border} ${style.surface} p-3`}>
      <p className={`text-sm font-bold ${style.text}`}>{lane.label}</p>
      {lane.caption && (
        ...
        <ol className="space-y-2">
          {spec.steps.map((step, index) => {
            ...
            return (
              ...
              >
                <div className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-bold ${style.badge}`}>
                  {index + 1}
                </div>
                <div>
                  {(from || to) && (
                  ...
                  <p className={`mt-0.5 text-sm font-bold ${style.text}`}>
                    {step.label}
                  </p>
                  {step.caption && (
                    <p className="mt-1 text-xs leading-relaxed text-gray-600">
      ...
      {lanes.map((lane, index) => (
        <div key={lane.id}>
          ...
          {index < lanes.length - 1 && (
            <div className="flex justify-center py-1 text-sm font-bold text-gray-400" aria-hidden>
              ↓
            </div>
          )}
        </div>
    ...
    <div className="space-y-3">
      {spec.canvasLabel && (
        <div className="rounded-lg bg-white/90 px-3 py-3 text-center ring-1 ring-white">
          <p className="text-sm font-bold text-gray-950">{spec.canvasLabel}</p>
        </div>
      )}
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
        {nodes.map((node) => (
...
function CycleHero({ spec }: { spec: HeroDiagramSpec }) {
  ...
  return (
    <div className="space-y-3">
      {cycle.center && (
        <div className="rounded-lg bg-white/90 px-3 py-3 text-center ring-1 ring-white">
          <p className="text-sm font-bold text-gray-950">{cycle.center}</p>
        </div>
      )}
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2">
                <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold ${style.badge}`}>
                  {index + 1}
                </span>
                <p className={`text-sm font-bold ${style.text}`}>{step.label}</p>
              </div>
              {step.caption && (
      ...
      <p className="rounded-lg bg-white/80 px-3 py-2 text-center text-xs font-bold text-gray-600 ring-1 ring-white">
        最後は次のPlanへ戻り、改善を続ける
      </p>
    </div>
  );
    ...
    <ul className="space-y-2">
      {links.map((link, index) => (
        <li
          ...
          className="rounded-lg bg-white/90 px-3 py-2 text-xs font-bold leading-relaxed text-gray-800 ring-1 ring-white"
        >
          {names.get(link.from) ?? link.from}
          <span className="mx-1 text-brand-500" aria-hidden>
            →
          </span>
