...
/** 次の抽選までの待ち時間（8〜20秒）。Reaction 後・復帰後もこの値から数え直す。 */
export function nextMacroIdleDelayMs(rng: RNG = Math.random): number {
  return Math.round(randRange(MACRO_IDLE_MIN_DELAY_MS, MACRO_IDLE_MAX_DELAY_MS, rng));
}
...
export function pickMacroIdleBehavior(
  ...
  const weights = macroIdleWeights(previous);
  const r = rng();
  let acc = 0;
...
export function macroIdleDurationMs(behavior: MochitMacroIdleBehavior, rng: RNG = Math.random): number {
  const [min, max] = MACRO_IDLE_DURATION_RANGE_MS[behavior];
 ...
 */
function stretchTracks(): ReactionTrack[] {
  ...
  return [
    {
      ...
      keyframes: [
        body(0, {}, EASE),
        body(0.22, { sx: 1.015, sy: 0.975 }, EASE),
        body(0.5, { sx: 0.988, sy: 1.032 }),
        body(0.7, { sx: 0.99, sy: 1.028 }, EASE),
    ...
    { target: "armR", keyframes: armKeys("R") },
    {
      target: "antenna",
      composite: "add",
      keyframes: [antenna(0, 0, EASE), antenna(0.5, -2, EASE), antenna(0.8, 1.2, EASE), antenna(1, 0)],
    },
  ];
}
...
export function buildMacroIdleSpec(behavior: MochitMacroIdleBehavior, options: MacroIdleSpecOptions): MacroIdleSpec {
  ...
  const tracks =
    behavior === "lookAround"
      ? lookAroundTracks(options.gazeBase ?? { x: 0, y: 0 })
      : behavior === "curious"
        ? curiousTracks(rng)
        : stretchTracks();
