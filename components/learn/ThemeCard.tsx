...
// 左端の状態線+章番号の色+テキスト(LessonStateBadge)で瞬時に判別できるようにする。
// 行頭には章のアプリアイコン(ThemeAppIcon)を置き、復習件数・習得済みはそのバッジで示す。
...
> = {
  ...
  not_started: { line: "bg-transparent", number: "text-gray-500" },
  in_progress: { line: "bg-brand-500", number: "text-brand-700" },
  review_due: { line: "bg-accent-500", number: "text-accent-700" },
  mastered: { line: "bg-emerald-500", number: "text-emerald-700" },
  fully_mastered: {
    line: "bg-emerald-700",
    ...
    >
      ...
      <div className="flex gap-3 p-4">
        <ThemeAppIcon
          theme={theme}
          size={48}
          className="mt-0.5 transition-transform duration-150 group-active:scale-95"
          badge={
            progress.reviewDueCount > 0
              ? { kind: "review", count: progress.reviewDueCount }
              : masterState === "fully_mastered"
                ? { kind: "fully_mastered" }
                : masterState === "mastered"
                  ? { kind: "mastered" }
                  : undefined
          }
        />
          ...
          >
            <span className="flex items-start justify-between gap-2">
              <span className="font-semibold text-gray-900 group-hover:text-brand-700">
                <span className={`mr-1.5 text-xs tabular-nums ${stateClass.number}`}>
      ...
      {isOpen && (
        <div id={panelId} className="border-t border-gray-100 px-4 py-3 sm:pl-[76px]">
