  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-4">
        <h2 className="text-sm font-extrabold text-gray-800">英単語の分解</h2>
        <p className="mt-1 text-sm text-gray-500">{entry.fullName}</p>
        <ul className="mt-3 space-y-1.5">
          {entry.words.map((w, i) => (
            <li key={`${w.word}-${i}`} className="flex items-baseline gap-2">
              <span className="rounded-md bg-indigo-50 px-2 py-0.5 text-sm font-bold text-indigo-700">
                {w.word}
              </span>
      ...
      <section className="rounded-2xl bg-indigo-600 p-5 text-white shadow-sm">
        <h2 className="text-xs font-bold text-indigo-200">一言でいうと</h2>
      ...
      {entry.examKeywords.length > 0 && (
        <section className="rounded-2xl bg-amber-50 p-4">
          <h2 className="text-sm font-extrabold text-amber-700">試験キーワード</h2>
      ...
      {entry.confusedWith.length > 0 && (
        <section className="rounded-2xl bg-rose-50 p-4">
          <h2 className="text-sm font-extrabold text-rose-600">似た語との違い</h2>
                ...
                >
                  {linked ? (
                    <Link
                      ...
                      className="text-sm font-extrabold text-rose-600 underline underline-offset-2"
                    >
                      {name} →
                    </Link>
                  ) : (
