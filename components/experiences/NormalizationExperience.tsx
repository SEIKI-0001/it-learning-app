  ...
  return (
    <div className="space-y-5">
      ...
      <Panel>
        ...
        {/* 更新異常を起こしてみる */}
        <div className="mt-4 rounded-xl bg-gray-50 p-3 ring-1 ring-gray-200" data-testid="price-lab">
          <div className="flex items-center justify-between gap-2">
            <p className="text-sm font-bold text-gray-800">試す：りんごを値上げ</p>
            <button
              type="button"
              onClick={() => setPriceUp((v) => !v)}
              aria-pressed={priceUp}
              className={`flex-none rounded-full px-3 py-1.5 text-xs font-bold transition active:scale-95 ${
          ...
          {priceUp && (
            <p
              ...
              className={`mt-2 rounded-lg px-3 py-2 text-xs leading-relaxed ring-1 ${
                normalized ? "bg-emerald-50 text-emerald-800 ring-emerald-200" : "bg-rose-50 text-rose-800 ring-rose-200"
              }`}
            >
              {normalized ? (
                <><b>商品表の1か所</b>を直すだけ。注文明細は商品番号で参照しているので、どの注文も自動的に120円になります。</>
              ) : (
                <>りんごの単価が<b>2か所</b>にある。1か所だけ直して、もう1か所を直し忘れると <b>120円と100円が食い違う</b>＝更新異常。</>
              )}
            </p>
          )}
        </div>
