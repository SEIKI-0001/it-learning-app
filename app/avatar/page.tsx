...
export default function AvatarPage() {
  ...
  return <main className="min-h-screen bg-gray-50 pb-24">
    ...
    <div className="mx-auto w-full max-w-md space-y-5 px-4 py-6 md:max-w-2xl">
      <section className="rounded-xl bg-white p-5 text-center border border-gray-200"><div className="flex justify-center"><Mochit state="normal" size="large" animation="idle" growthStage={stage} /></div><p className="mt-2 text-base font-bold text-gray-800">成長段階：{MOCHIT_GROWTH_STAGE_LABELS[stage]}</p><p className="mt-1 text-xs font-semibold text-gray-500">チェックポイント {summary.clearedCheckpointCount}回クリア・バッジ {summary.earnedBadgeCount}個</p>{nextStage ? <p className="mt-3 rounded-xl bg-brand-50 px-3 py-2 text-sm font-bold text-brand-800">次の成長条件：{nextStage.conditionLabel}</p> : <p className="mt-3 rounded-xl bg-brand-50 px-3 py-2 text-sm font-bold text-brand-800">すべての成長段階を解放済み</p>}</section>
      <section className="rounded-xl bg-white p-5 border border-gray-200"><h2 className="text-base font-bold text-gray-800">解放済みの記録</h2><p className="mt-1 text-sm text-gray-500">知識コアの点灯と成長は、クリア済みチェックポイントで保たれます。</p><p className="mt-3 text-sm font-bold text-brand-700">現在地：{cp.currentCheckpointId.toUpperCase()}</p></section>
